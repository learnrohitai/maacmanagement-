...
import { COURSE_DATABASE, findSoftwareDetails, calculateStudentCourseProgress, calculateBatchTracker, calculateCourseTracker, getSoftwaresForCourse, StudentCourseProgressReport } from '@/lib/softwareData';
...
export default function AcademicManagerDashboard() {
  ...
  // ===== Course Progress Report (dedicated report section) =====
  const [cprCourse, setCprCourse] = useState<string>(COURSE_DATABASE[0]?.name || '');
  const [cprSearch, setCprSearch] = useState('');
  const [cprView, setCprView] = useState<'software' | 'student'>('software');
  ...
  // ===== Course Progress Report computations =====
  const cprSoftwares = useMemo(() => getSoftwaresForCourse(cprCourse), [cprCourse]);

  const cprStudents = useMemo(
    () => allStudents.filter(s => s.course === cprCourse),
    [allStudents, cprCourse]
  );

  const cprReports = useMemo(() => {
    const map = new Map<string, { student: UserType; report: StudentCourseProgressReport }>();
    cprStudents.forEach(s => {
      map.set(s.id, {
        student: s,
        report: calculateStudentCourseProgress(s.id, s.course, attendance, s.course, batches),
      });
    ...
    return map;
  }, [cprStudents, attendance, batches]);

  // Aggregate per-software completion across all students of the course
  const cprSoftwareStats = useMemo(() => {
    return cprSoftwares.map(sw => {
      let attendedSum = 0;
      let started = 0;
      let completed = 0;
      cprReports.forEach(({ report }) => {
      ...
      const studentCount = cprReports.size || 1;
      return {
        softwareId: sw.id,
        ...
        totalStudents: cprReports.size,
      };
    });
  }, [cprSoftwares, cprReports]);

  const cprTotals = useMemo(() => {
    let totalSessions = 0;
  ...
  // Chart data: avg completion per software (for the report bar chart)
  const cprChartData = useMemo(
    () => cprSoftwareStats.map(sw => ({
  ...
  const cprFilteredReports = useMemo(() => {
    const term = cprSearch.toLowerCase();
    return Array.from(cprReports.values()).filter(({ student }) =>
      !term ||
      student.name.toLowerCase().includes(term) ||
      (student.studentId || '').toLowerCase().includes(term)
    );
  }, [cprReports, cprSearch]);
      ...
      <Card className="p-6 border-l-4 border-l-emerald-500">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-bold text-gray-900">Course Progress Report</h3>
            </div>
            <p className="text-xs text-gray-500 mt-0.5">
              Software-wise &amp; student-wise progress report for every course program — sessions attended vs. remaining, per software.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            <select
              value={cprCourse}
              onChange={(e) => setCprCourse(e.target.value)}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-5">
          <div className="p-3.5 rounded-2xl bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-200/70">
            ...
            <p className="text-2xl font-extrabold text-emerald-700 leading-tight">{cprTotals.overallCompletion}%</p>
            <p className="text-[10px] text-gray-500">{cprTotals.attendedSessions}/{cprTotals.totalSessions} sessions</p>
          </div>
          <div className="p-3.5 rounded-2xl bg-gray-50 border border-gray-200">
            ...
            <p className="text-2xl font-extrabold text-gray-800 leading-tight">{cprTotals.totalStudents}</p>
            <p className="text-[10px] text-gray-500">in {cprCourse}</p>
          </div>
          <div className="p-3.5 rounded-2xl bg-gray-50 border border-gray-200">
            <p className="text-[10px] text-gray-500 font-semibold uppercase">Softwares Completed</p>
            <p className="text-2xl font-extrabold text-emerald-600 leading-tight">{cprTotals.completedSoftwares}</p>
          ...
          <div className="p-3.5 rounded-2xl bg-gray-50 border border-gray-200">
            ...
            <p className="text-2xl font-extrabold text-amber-600 leading-tight">{cprTotals.inProgressSoftwares}</p>
          ...
          <div className="p-3.5 rounded-2xl bg-gray-50 border border-gray-200">
            ...
            <p className="text-2xl font-extrabold text-gray-500 leading-tight">{cprTotals.pendingSoftwares}</p>
          ...
          <div className="p-3.5 rounded-2xl bg-gray-50 border border-gray-200">
            ...
            <p className="text-2xl font-extrabold text-purple-700 leading-tight">
              {cprTotals.studentsOnTrack}
              <span className="text-sm text-gray-400 font-bold"> / </span>
              {cprTotals.studentsAtRisk}
            </p>
            <p className="text-[10px] text-gray-500">≥50% / &lt;50% started</p>
          </div>
        </div>

        {/* Chart: avg completion per software */}
        <div className="mb-6">
          <p className="text-xs font-bold text-gray-600 uppercase tracking-wide mb-2">
            Average Completion by Software — {cprCourse}
          </p>
          {cprChartData.length === 0 ? (
            <div className="p-6 text-center text-xs text-gray-500 bg-gray-50 rounded-xl">
            ...
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={cprChartData} margin={{ top: 8, right: 12, left: -18, bottom: 0 }}>
        ...
        {cprView === 'software' ? (
          /* ===== Software-wise report table ===== */
          <div>
            <p className="text-xs font-bold text-gray-600 uppercase tracking-wide mb-2">
              Software-wise Progress Report — {cprCourse}
            </p>
            {cprSoftwareStats.length === 0 ? (
              <div className="p-6 text-center text-xs text-gray-500 bg-gray-50 rounded-xl">
                No softwares mapped for this course.
              </div>
            ) : (
                    ...
                    {cprSoftwareStats.map((sw) => {
                      ...
                      return (
                        <tr key={sw.softwareId} className="hover:bg-gray-50/60">
                          <td className="py-2.5 px-3">
                            <div className="text-xs font-bold text-gray-900">{sw.softwareName}</div>
                          </td>
          ...
          <div>
            <p className="text-xs font-bold text-gray-600 uppercase tracking-wide mb-2">
              Student-wise Progress Report — {cprCourse} ({cprFilteredReports.length})
            </p>
            {cprFilteredReports.length === 0 ? (
              <div className="p-6 text-center text-xs text-gray-500 bg-gray-50 rounded-xl">
                No students enrolled in <strong>{cprCourse || 'this course'}</strong> yet.
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {cprFilteredReports.map(({ student, report }) => (
                      <tr key={student.id} className="hover:bg-gray-50/60">
                        <td className="py-2.5 px-3">
